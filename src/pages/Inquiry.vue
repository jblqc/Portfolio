<template>
	<div class="inquiry-container">
		<Text
			...
			class="tracking-wider custom-font" /><br />
		<Text
		...
		<v-form
			...
			<TextField
				...
				hide-details
				:error="!!errors.name"
				:error-messages="errors.name"></TextField>
			<Text
				:text="'Email'"
				variant="subtitle-2"
				fontWeight="500" />
			...
			<div
				v-if="inquiryStore.selectedProjects.length > 0"
				class="mt-5">
				...
				<v-btn-toggle
					...
					:style="
						isDarkMode
							? 'border: 1px solid #808080; color: rgba(129, 114, 131, 0.993);'
							: ''
					">
					<v-btn
...
<script setup>
	...
	const validateEmail = () => {
		...
		if (!email) {
			...
			errors.value.email =
				"This doesn't seem to be a valid email. Please check and try again.";
		} else {
	...
	const validateForm = () => {
		errors.value = {
			...
			email: inquiryStore.email ? '' : 'Email is required',
			...
			budget: inquiryStore.budget ? '' : 'Budget is required',
